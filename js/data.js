...
/* 특정 주(월요일 시작)의 내 운동 인증 요약 */
function myWeekSummary(weekStartDate, exMap) {
  exMap = exMap || dayTimeMap();
  const exPh = state.dayPhotos || {};
  const ms = weekStartDate || mondayStart();
  const days = [];
  for (let i = 0; i < 7; i++) {
    const d = new Date(ms); d.setDate(d.getDate() + i); const ds = todayStr(d);
    days.push({ date: ds, exSecs: exMap[ds] || 0, exPhoto: exPh[ds] || null });
  }
  const exDays = days.filter(d => d.exSecs > 0 || d.exPhoto).length;
  const certDays = exDays;
  const exPhotos = days.filter(d => d.exPhoto).length;
  const totalEx = days.reduce((a, d) => a + d.exSecs, 0);
  const goal = state.settings.weeklyGoal || 3;
  const score = exDays + exPhotos;  // 사진 인증하면 점수 UP
  return {
    start: todayStr(ms), days, exDays, certDays, exPhotos, totalEx, goal,
    done: certDays >= goal, score,
    // 하위호환
    workoutDays: certDays, photoDays: exPhotos, totalSecs: totalEx
  };
}
/* 최근 N주치 (챌린지 푸시용) */
