/* 알려줄까말까 — 앱 로직 (탭 네비게이션, 세션 기록, 휴식 타이머, 루틴, 캘린더, 통계) */

/* ===== 진행 중 세션 (리로드에도 살아남도록 별도 저장) ===== */
const ACTIVE_KEY = 'almalka.active';
...
function render() {
  ...
  else if (curTab === 'diet') renderDiet();
...
function renderHome() {
  ...
  el.innerHTML = `
    <header class="home-head">
      ...
      <p class="tagline">${streak > 0 ? `🔥 ${streak}일 연속 인증 중이에요!` : '오늘도 운동으로 몸 챙겨볼까요? 💪'}</p>
    </header>
    <div class="stat-row">
    ...
    <div class="home-tools">
      <button id="btn-formcheck" class="tool-btn">🤖 AI 자세 체크</button>
  ...
  el.querySelector('#btn-formcheck')?.addEventListener('click', () => aiFormCheck());
  el.querySelector('#btn-1rm')?.addEventListener('click', () => open1RM());
...
function myCertCard(sum) {
  ...
  return `<div class="cert-card">
    ...
    <p class="cg-tip">🖐️ 요일을 눌러 <b>운동 인증</b>! (시간만 톡, 사진 선택)</p>
    <div class="cert-metrics">
      <div><span>💪 운동</span><b data-count="${sum.exDays}">0</b><small>일 · ${fmtHM(sum.totalEx) || '0:00'}</small></div>
      <div><span>📷 사진</span><b data-count="${sum.exPhotos}">0</b><small>일</small></div>
...
function openQuickDay(ds) {
  ...
  let pEx = null, rmEx = false, busy = false;
  ...
  const draw = () => {
    m.querySelector('#qd-body').innerHTML = `
      <p class="qd-guide">오늘 <b>운동</b>을 인증하세요. 사진 올리면 순위 +1점! 🔥</p>
      ${sect('ex', '💪', '오늘 운동', curEx, curExP, exPre, pEx, rmEx)}
      <button id="qd-save" class="big-btn">${busy ? '저장 중…' : '인증하기'}</button>
      ${(curEx || curExP) ? '<button id="qd-clear" class="text-btn danger">이 날 인증 전체 지우기</button>' : ''}`;
    ...
    m.querySelectorAll('[data-del]').forEach(b => b.addEventListener('click', () => { pEx = null; rmEx = true; draw(); }));
  ...
  const doSave = async () => {
    if (busy) return;
    const exMin = parseInt(m.querySelector('#qd-ex-min').value) || 0;  // 입력값을 draw() 전에 캡처
    busy = true; draw();
    try { await applyPhoto(pEx, rmEx, 'ex', 'dayPhotos'); }
  ...
  const clearDay = () => {
    if (!confirm('이 날 인증(운동·사진)을 모두 지울까요?')) return;
    ['manualDays', 'dayPhotos'].forEach(k => { if (state[k]) delete state[k][ds]; });
...
function attSheet(rows) {
  ...
  const body = rows.map(r => {
    ...
    const cells = r.days.map(d => {
      const exOn = d.exSecs > 0 || d.exPhoto, anyPhoto = d.exPhoto;
      if (!exOn) return `<td class="att-cell"></td>`;
      return `<td class="att-cell has ${anyPhoto ? 'ph' : ''}" ${anyPhoto ? `data-photo="${d.exPhoto}"` : ''}>
        <div class="att2">${cell(d.exSecs, d.exPhoto, '💪', 'ex')}</div></td>`;
    }).join('');
    return `<tr class="${r.isMe ? 'me' : ''}"><td class="att-name"><b>${esc(r.nick || '익명')}</b>${meta ? `<small>${esc(meta)}</small>` : ''}</td>${cells}<td class="att-sum"><b>${r.certDays}일</b><small>💪${r.exDays}</small>${r.photoCnt ? `<i>📷${r.photoCnt}</i>` : ''}</td></tr>`;
  }).join('');
  return `<div class="att-wrap"><table class="att"><thead>${head}</thead><tbody>${body}</tbody></table></div>
    <p class="lb-legend">💪운동 · 📷 있는 칸을 눌러 사진 보기 · <b>사진 인증하면 순위 UP</b></p>`;
}

/* 리더보드 행 계산: 멤버별 운동 요약 + 점수 랭크 */
function challengeBoard(ms) {
  ...
  const rows = members.map(m => {
    const days = []; let total = 0, exDays = 0, certDays = 0, photoCnt = 0, lastPhoto = null;
    const exT = m.dayTimes || {}, exP = m.photos || {};
    for (let i = 0; i < 7; i++) {
      const d = new Date(ms); d.setDate(d.getDate() + i); const ds = todayStr(d);
      const es = exT[ds] || 0, ep = exP[ds] || null;
      days.push({ exSecs: es, exPhoto: ep });
      const exd = es > 0 || ep;
      if (exd) { exDays++; certDays++; }
      total += es;
      if (ep) { photoCnt++; lastPhoto = ep; }
    }
    const score = exDays + photoCnt;
    return { ...m, days, exDays, certDays, cnt: certDays, total, photoCnt, lastPhoto, score, done: certDays >= goal };
  });
...
function shareCertImage(sum) {
  ...
  for (let i = 0; i < 7; i++) {
    const cx = gx + i * cw, dsecs = sum.days[i].exSecs, on = dsecs > 0 || sum.days[i].exPhoto;
    ...
    x.fillStyle = on ? '#06291F' : '#5E6675'; x.font = DISP(17); x.fillText(fmtHM(dsecs) || (sum.days[i].exPhoto ? '📷' : '·'), cx + cw / 2, gy + 63);
  }
  x.textAlign = 'left';
  // 지표 — 흰색 대형 숫자
  ...
  cv.toBlob(async blob => {
    ...
    const file = new File([blob], `${BRAND.en || 'workout'}-인증-${sum.start}.png`, { type: 'image/png' });
...
function logoSvg(cls) {
  return `<svg class="brand-logo ${cls || ''}" viewBox="0 0 100 100" aria-hidden="true">
    ...
    <g fill="url(#bf-grad)">
      ...
      <rect x="14" y="32" width="12" height="36" rx="4"/>
      <rect x="26" y="38" width="7" height="24" rx="3"/>
      <rect x="74" y="32" width="12" height="36" rx="4"/>
      <rect x="67" y="38" width="7" height="24" rx="3"/>
    </g>
  </svg>`;
}
function dateKicker() {
...
const APP_VER = '1.0.0';
function init() {
  document.title = `${BRAND.name} — 운동 인증 습관`;
...
function showWelcome() {
  ...
  m.querySelector('#wc-body').innerHTML = `
    <div class="wc-hero">
      ...
      <h2>${BRAND.name}에 오신 걸 환영해요!</h2>
      <p>매일 운동을 인증하고,<br>운동방 친구들과 함께 습관을 만드는 앱이에요.</p>
    </div>
    <div class="wc-steps">
      <div class="wc-step"><span class="wc-num">1</span><div><b>챌린지 탭에서 인증</b><span>요일을 눌러 오늘 운동 시간을 톡!</span></div><span class="wc-ic">🗓️</span></div>
      <div class="wc-step"><span class="wc-num">2</span><div><b>사진 인증</b><span>운동 사진 올리면 순위 +1점 🔥</span></div><span class="wc-ic">📷</span></div>
      <div class="wc-step"><span class="wc-num">3</span><div><b>출석부·리더보드</b><span>꾸준히 한 사람이 위로! 서로 확인해요</span></div><span class="wc-ic">🏅</span></div>
    </div>
...
function openHelp() {
  ...
  m.querySelector('#help-body').innerHTML = `
    <p class="help-lead">${BRAND.name} = <b>매일 운동 인증</b>! 1분이면 익혀요 👇</p>
    ...
    ${sec('📷', '사진 인증 = 순위 UP', `<p>운동 <b>사진</b>을 올리면 <b>순위 +1점</b> 🔥 아무 사진이나 누르면 <b>모두의 인증 사진</b>을 갤러리로 넘겨볼 수 있어요.</p>`)}
